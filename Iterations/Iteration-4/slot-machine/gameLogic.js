...
const GameLogic = (() => {
   ...
   * If a real entry for this player already exists, the session stats are
   * accumulated so the leaderboard reflects total points across all sessions.
   * Noop if no spins have been made.
  ...
  function saveScore() {
    ...
    const existing = board.findIndex(e => !e._sim && e.name === _state.playerName);

    if (existing >= 0) {
      const prev = board[existing];
      board[existing] = {
        name:       _state.playerName,
        avatarId:   _state.avatarId ?? prev.avatarId,
        totalSpins: prev.totalSpins + _state.totalSpins,
        totalWon:   prev.totalWon   + _state.totalWon,
        biggestWin: Math.max(prev.biggestWin, _state.biggestWin),
        netProfit:  prev.netProfit  + (_state.balance - STARTING_BALANCE),
        timestamp:  Date.now(),
      };
    } else {
      board.push({
