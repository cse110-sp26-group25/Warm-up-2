...
(function () {
  ...
  // ── Reel strip setup ──────────────────────────────────────────────────
  /**
   * Height of a single symbol slot in pixels.
   * Must match the CSS custom property --symbol-h (80px).
   * @type {number}
   */
  const SYMBOL_HEIGHT = 80;

  /**
   * Number of symbol copies duplicated in each strip.
   * 5 reps ensures the scroll animation never outruns the strip content.
   * @type {number}
   */
  const STRIP_REPS = 5;

  /**
   * Build the DOM symbol strip for one reel.
   * @param {number} reelIndex - Index of the reel (0–4).
   * @returns {void}
   */
  ...
  // ── Spin animation ─────────────────────────────────────────────────────
  /**
   * Animate one reel strip to its resolved stop, centering the target
   * symbol in the middle (payline) row of the 3-row visible window.
   *
   * Snap position formula: `(total + targetStop - 1) * SYMBOL_HEIGHT`
   *  • Row 0 (top)    → symbol at (targetStop - 1) mod total
   *  • Row 1 (center) → symbol at targetStop          ← payline
   *  • Row 2 (bottom) → symbol at (targetStop + 1) mod total
   *
   * @param {number} reelIndex    - Reel column index (0–4).
   * @param {number} targetStop   - Strip index of the winning center symbol.
   * @param {number} spinDuration - CSS transition duration in milliseconds.
   * @returns {Promise<void>} Resolves when the reel has settled.
   */
  function _animateReel(reelIndex, targetStop, spinDuration) {
    return new Promise(resolve => {
      ...
      const reel  = GameLogic.REELS[reelIndex];
      const total = reel.length;
      const symH  = SYMBOL_HEIGHT; // 80px — must match CSS --symbol-h

      // Start position: random symbol centered in the 2nd strip repetition
      const startPos  = (total + RNG.randInt(0, total - 1) - 1) * symH;

      // Target position: targetStop centered (row 1) in 2nd repetition
      const targetPos = (total + targetStop - 1) * symH;

      // One extra full rotation for spin effect; stagger by reel index
      // is handled via spinDuration (each reel gets a longer duration).
      const finalPos  = targetPos + total * symH;
      ...
      setTimeout(() => {
        // Snap to clean 2nd-rep centered position (avoids accumulated offset)
        strip.style.transition = 'none';
  ...
  async function _doSpin() {
    ...
    // Reel durations staggered across 5 reels for dramatic sequential stop
    const durations = [800, 1050, 1300, 1550, 1800];
