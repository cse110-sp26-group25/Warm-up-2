...
- A fast play system, has legal limitations but it good for keeping retention for impatient players or users who only care about money. Provides the user a sence of control. 
- The roll should only take 3-5 seconds to keep suspect / exitment for those who want entertainment without being too intrusive to those who don't. Also make **BIG WINS** be longer and a litte more drawn out
  - Include paylines, bonus rounds, pot system, pity system (rewards and enables the Near-Miss effeect) etc... to offer incentives the appeal to a variety of different personas. Entertainment wise it breaks up the monotonay. Economically it gives a "free" reward. Provides reasons (including sunk cost fallacy) to keep playing even if one has been losing recently.
- Displays for important stuff to the user to consider (eg. Money counter, Money Won, settings etc.) should be on the screen, but not the main focus.
...
- Useful to exagarate small wins into making them more significant, works well along side loss aversion, which tricks users into believing they won more than they lost
- Have animations and themes for the slot machine that can be changed to keep the users engaged for longer
- Have themes branch out to different medias (Games, Movies, TV shows, sports, etc.) along with its basic themes (Classic, Egyption, Fantasy, etc.)
- Have little things go on keep the user playing and less tired.
...
- Having a ranking and leaderboard to encourage competition to also fuel engagement. This can be further utilized through the implementation of avatar creation and text/voice rooms that would also have to be monitored

#### Extra Information to Consider
- Make the code hidden and unacessable through dev tools
- Implementing side quests can be useful for keeping players on, even if its for like 15 minutes (Short engagement is still engagement). 
